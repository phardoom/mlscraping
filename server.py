...
# Configurar rotas dos módulos da API
...
async def index() -> HTMLResponse:
    """Rota raiz que serve o template HTML."""
    html_path = Path("templates/index.html")
    if html_path.exists():
        return HTMLResponse(content=html_path.read_text(encoding="utf-8"))
    return HTMLResponse(content="<h1>Template não encontrado</h1>", status_code=500)
...
async def status() -> dict:
    """Retorna status do scraping (compatibilidade com código antigo)."""
